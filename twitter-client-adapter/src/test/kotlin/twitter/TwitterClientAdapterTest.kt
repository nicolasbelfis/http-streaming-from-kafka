...
internal class TwitterClientAdapterTest {
    ...
    fun `retry flux example,fails then retries then rethrow a new exception on exhaust`() {
        ...
        val retryWhen = TwitterClientAdapter(twitterClient).multicastStream()
            .retryWhen(
                Retry.indefinitely()
                    .maxAttempts(1L)
    ...
    fun `should open only one connexion at first subscription`() {
        ...
        val flux = TwitterClientAdapter(twitterClientMock).multicastStream().log()
        flux.subscribe()
