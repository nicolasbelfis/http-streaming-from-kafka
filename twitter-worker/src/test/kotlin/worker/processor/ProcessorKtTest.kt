...
internal class ProcessorKtTest {
    ...
    private val twitterWorker = mockk<TwitterWorker>()
    private val mockProducer = mockk<ReactiveProducer>()
    @Test
    fun `should process all element when producer works`() {

        every { twitterWorker.stream() } returns Flux.just(FakeTweet("1"), FakeTweet("2"))
        ...
        val fluxToTest = Processor(1,1,1).run(twitterWorker, mockProducer)
    ...
    fun `should discard elements that were pushed down stream but not subscribed`() {
        ...
        every { twitterWorker.stream() } returns Flux.just(FakeTweet("1"), FakeTweet("2"))
        every { mockProducer.sendTweetToKafka(FakeTweet("1")) } returns Mono.defer<RecordMetadata?> { Mono.error(Exception()) }.subscribeOn(Schedulers.single())
        val fluxToTest = Processor(1,1,1).run(twitterWorker, mockProducer)
    ...
    fun `should discard on backpressure`() {
        ...
        every { twitterWorker.stream() } returns sevenTweets()
        every { mockProducer.sendTweetToKafka(FakeTweet("1")) } returns Mono.delay(Duration.ofSeconds(2)).map { println("tweet $it");fakeRecordWithOffset(1L) }
        every { mockProducer.sendTweetToKafka(FakeTweet("2")) } returns Mono.delay(Duration.ofSeconds(10)).map { println("tweet $it");fakeRecordWithOffset(2L) }
        every { mockProducer.sendTweetToKafka(FakeTweet("3")) } returns Mono.delay(Duration.ofSeconds(2)).map { fakeRecordWithOffset(3L) }
        val fluxToTest = Processor(1,1,0)
            .run(twitterWorker, mockProducer)
